...
The second move, particularly in analysis, is to draw a picture — even a rough one. The $\varepsilon$-$\delta$ definition of a limit becomes significantly less intimidating once you understand it geometrically: $f(x)$ landing within a vertical band whenever $x$ is within a horizontal band around $c$. If you can see what you want to happen, you are already most of the way to expressing it formally.
